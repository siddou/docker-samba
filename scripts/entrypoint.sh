...
EOF
  ...
  for I_GROUP in "${arr[@]}"
  do
    GROUP_NAME=$(echo "$I_GROUP" | cut -d'=' -f1 | sed 's/GROUP_//g' | tr '[:upper:]' '[:lower:]')
    GROUP_MEMBERs=$(echo "$I_GROUP" | sed 's/^[^=]*=//g')

    echo ">> GROUP: adding GROUP: $GROUP_NAME"
    groupadd $GROUP_NAME
    echo ">> $GROUP_NAME : adding member: $GROUP_MEMBERs"
    gpasswd -M $GROUP_MEMBERs $GROUP_NAME 
